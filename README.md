...
1. `tool/story_template.txt` をコピーして `assets/stories_src/<id>.txt` を作成し、本文を書く
   (書式は `tool/story_template.txt` 内のコメント兼サンプルを参照。画面上のページ分割は端末サイズに合わせてアプリが自動で行うため、
   本文を続けて書くだけでよい。空行は段落区切りとして反映される。一文ごとに空行がある原稿は、読みやすい段落へ自動で結合される)
2. 変換コマンドを実行:
   ```powershell
   cd kotonoha
...
```json
{
  ...
  "pages": ["本文。画面上のページ分割はアプリが自動で行います。"],
  "column": {
    "trivia": ["豆知識1", "豆知識2"],
    "quiz": [
      {
...
`column` は省略可。本文は `pages` 配列でも `text_file` で指定した `.txt` でも読み込めるが、
どちらも同じ縦書き表示・自動ページ分割が適用される。
